...
func NewToolsManager(toolGrantsModel model.ToolGrantsModel, agentsModel model.AgentsModel, memoryStore contracts.SqlMemoryStore, memoryDocumentsModel model.MemoryDocumentsModel, agentContextFilesModel model.AgentContextFilesModel, skillsModel model.SkillsModel, skillGrantsModel model.SkillGrantsModel, ossClient *oss.Client, redisClient *redis.Redis) contracts.ToolsManagerInterface {
	return &ToolsManager{
...
func (t ToolsManager) tools(ctx context.Context, userId, agentID string) []toolDef {
	...
	if userId == "" {
		agent, _ := t.agentsModel.FindByAgentID(ctx, agentID)
		if agent != nil {
			userId = agent.UserId
		}
	}
	...
	// use_skill tool: per-agent skill loader
	if t.skillsModel != nil && t.skillGrantsModel != nil {
		loader := skills.NewDBSkillsLoader(t.skillsModel, t.skillGrantsModel, t.ossClient, t.redisClient, agentID, userId)
		toolDefs = append(toolDefs,
			toolDef{
				name:    "use_skill",
				enabled: true,
				builder: func() tool.BaseTool {
					return utils.WrapInvokableToolWithErrorHandler(NewUseSkillTool(loader), errorHandler)
				},
			},
			toolDef{
				...
				enabled: true,
				builder: func() tool.BaseTool {
					return utils.WrapInvokableToolWithErrorHandler(NewReadSkillFileTool(loader), errorHandler)
				},
			},
			toolDef{
				...
				enabled: true,
				builder: func() tool.BaseTool {
					return utils.WrapInvokableToolWithErrorHandler(NewRunSkillScriptTool(loader), errorHandler)
				},
			},
		)
	}
