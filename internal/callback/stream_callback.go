...
func NewStreamCallback(client bus.Streamer, sessions contracts.SessionManagerInterface, sessionId int64, agentID string) callbacks.Handler {
	cb := &StreamCallback{
		sessions:  sessions,
		client:    client,
		sessionId: sessionId,
		...
		for {
			...
			case []*schema.Message:
				...
				for _, m := range v {
					...
					cb.sessions.AddFullMessage(cb.agentID, cb.sessionId, *m)
				}
		...
		if len(msgs) > 0 {
			...
			if err != nil {
				...
				if msg.Content != "" || len(msg.ToolCalls) > 0 {
					cb.sessions.AddFullMessage(cb.agentID, cb.sessionId, *msg)
