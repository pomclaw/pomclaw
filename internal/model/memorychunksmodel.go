...
func (m *customMemoryChunksModel) FindByAgentIdAndPath(ctx context.Context, agentId string, path string) ([]*MemoryChunks, error) {
	...
	query := `select id, document_id, agent_id, path, start_line, end_line, hash, text, embedding, tsv, custom_scope, created_at, updated_at from "public"."memory_chunks" where agent_id = $1 and path = $2 order by start_line asc`
