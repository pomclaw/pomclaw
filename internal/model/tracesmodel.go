...
// FindByOtelTraceID finds a trace by the OpenTelemetry trace id stored in metadata.
func (m *customTracesModel) FindByOtelTraceID(ctx context.Context, traceID string) (*Traces, error) {
	query := `SELECT * FROM "public"."traces" WHERE metadata->>'otel_trace_id' = $1 LIMIT 1`
	var resp Traces
