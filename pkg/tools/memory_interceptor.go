...
type MemoryInterceptor struct {
	memStore  model.MemoryDocumentsModel
	workspace string
}

// NewMemoryInterceptor creates an interceptor backed by the given memory store.
func NewMemoryInterceptor(ms model.MemoryDocumentsModel, workspace string) *MemoryInterceptor {
...
func (m *MemoryInterceptor) ReadFile(ctx context.Context, path string) (string, bool, error) {
	...
	content, err := m.memStore.GetDocument(ctx, agentID, userID, relPath)
	if err != nil && userID != "" {
		content, err = m.memStore.GetDocument(ctx, agentID, "", relPath)
	}
	if err != nil {
		...
		slog.Debug("memory interceptor: document not found", "path", path, "agent", agentID)
		return "", true, nil
	}
...
func (m *MemoryInterceptor) WriteFile(ctx context.Context, path, content string, appendMode bool) (MemoryWriteResult, error) {
	...
	if appendMode {
		...
		existing, err := m.memStore.GetDocument(ctx, agentID, userID, relPath)
		if err == nil && existing != "" {
			content = existing + "\n\n---\n\n" + content
		}
	} else {
		// Replace: capture previous content for overwrite warning.
		oldContent, err := m.memStore.GetDocument(ctx, agentID, userID, relPath)
	...
	if err := m.memStore.PutDocument(ctx, agentID, userID, relPath, content); err != nil {
		return MemoryWriteResult{Handled: true}, err
	}

	// Only index .md files (chunk + embed).
	if strings.HasSuffix(relPath, ".md") {
		if err := m.memStore.IndexDocument(ctx, agentID, userID, relPath); err != nil {
...
func (m *MemoryInterceptor) ListFiles(ctx context.Context, path string) (string, bool, error) {
	...
	userID := MemoryUserID(ctx)
	docs, err := m.memStore.ListDocuments(ctx, agentID, userID)
	if err != nil {
		return "", true, err
	}
