...
func Run(opts Options) (*Result, error) {
	...
	pomclawHome, err := resolvePomclawHome(opts.PomclawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, pomclawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to Pomclaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, pomclawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, pomclawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(pomclawHome, "config.json"),
				Description: "convert OpenClaw config to Pomclaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(pomclawHome)
...
func Execute(actions []Action, openclawHome, pomclawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, pomclawHome); err != nil {
		...
		case ActionBackup:
			...
			if err := copyFile(action.Source, action.Destination); err != nil {
				...
				result.FilesCopied++
				fmt.Printf("  ✓ Copied %s\n", relPath(action.Source, pomclawHome))
			}
		case ActionCopy:
			if err := os.MkdirAll(filepath.Dir(action.Destination), 0755); err != nil {
				result.Errors = append(result.Errors, err)
...
func executeConfigMigration(srcConfigPath, dstConfigPath, pomclawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing Pomclaw config: %w", err)
		}
...
func resolvePomclawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("POMCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".pomclaw"), nil
}
