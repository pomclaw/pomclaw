...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'deepseek' not supported in Pomclaw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in Pomclaw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.pomclaw/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.pomclaw/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.pomclaw/workspace"},
...
func TestRunDryRun(t *testing.T) {
	pomclawHome := t.TempDir()
	pomclawHome := t.TempDir()

	wsDir := filepath.Join(pomclawHome, "workspace")
	...
	os.WriteFile(filepath.Join(pomclawHome, "openclaw.json"), data, 0644)

	opts := Options{
		DryRun:       true,
		OpenClawHome: pomclawHome,
		PomclawHome: pomclawHome,
	}
	...
	picoWs := filepath.Join(pomclawHome, "workspace")
	if _, err := os.Stat(filepath.Join(picoWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(pomclawHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	picoConfig, err := config.LoadConfig(filepath.Join(pomclawHome, "config.json"))
	if err != nil {
		t.Fatalf("loading Pomclaw config: %v", err)
	}
