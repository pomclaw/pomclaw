...
// VectorSearch performs a generic vector similarity search on a table using PostgreSQL pgvector.
...
func VectorSearch(db *sql.DB, table, idCol, textCol, embeddingCol, agentID string, queryVector []float32, maxResults int, minScore float64) ([]VectorSearchResult, error) {
	...
	// PostgreSQL pgvector: convert []float32 to JSON string for the <=> operator
	vecStr := float32SliceToString(queryVector)
	...
	rows, err := db.Query(query, vecStr, agentID, maxResults)
	if err != nil {
