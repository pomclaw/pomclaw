...
func DownloadFile(url, filename string, opts DownloadOptions) string {
	...
	mediaDir := filepath.Join(os.TempDir(), "pomclaw_media")
	if err := os.MkdirAll(mediaDir, 0700); err != nil {
		logx.Error(opts.LoggerPrefix, "Failed to create media directory", map[string]interface{}{
			"error": err.Error(),
		})
		return ""
	}
	...
	if err != nil {
		logx.Error(opts.LoggerPrefix, "Failed to create download request", map[string]interface{}{
			"error": err.Error(),
		})
		return ""
	}
	...
	if err != nil {
		logx.Error(opts.LoggerPrefix, "Failed to download file", map[string]interface{}{
			"error": err.Error(),
			"url":   url,
		})
		return ""
	...
	if resp.StatusCode != http.StatusOK {
		logx.Error(opts.LoggerPrefix, "File download returned non-200 status", map[string]interface{}{
	...
	if err != nil {
		logx.Error(opts.LoggerPrefix, "Failed to create local file", map[string]interface{}{
			"error": err.Error(),
		})
		return ""
	}
	...
	if _, err := io.Copy(out, resp.Body); err != nil {
		out.Close()
		os.Remove(localPath)
		logx.Error(opts.LoggerPrefix, "Failed to write file", map[string]interface{}{
			"error": err.Error(),
		})
		return ""
	}

	logx.Debug(opts.LoggerPrefix, "File downloaded successfully", map[string]interface{}{
		"path": localPath,
	})
