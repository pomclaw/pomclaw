# OCI Generative AI Integration for Pomclaw

This module provides **OCI Generative AI** as an optional LLM backend for Pomclaw. It runs a local OpenAI-compatible proxy that authenticates with OCI using your `~/.oci/config` credentials and forwards requests to the OCI GenAI inference endpoint.
...
4. **Configure Pomclaw** (`~/.pomclaw/config.json`):
   ```json
   {
     "agents": {
       "defaults": {
...
```
Pomclaw (Go) --> localhost:9999/v1 (proxy.py) --> OCI GenAI endpoint
