// WebSocket v3 client for Pomclaw gateway RPC protocol
// Adapted from GoClaw with Phase 1 simplifications (no token auth)
...
export class WsClient {
  ...
  constructor(config: WsClientConfig) {
    ...
    this.senderId = config.senderId || 'web'
    this.locale = config.locale || navigator.language.split('-')[0] || 'en'
  }
