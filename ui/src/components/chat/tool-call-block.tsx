...
import type { ToolCall } from '@/store/chat-messages'
...
function ToolIcon({
  ...
  if (isSkill) {
    if (state === 'calling')
      return <ZapIcon className={`${cls} animate-pulse text-amber-500`} />
    if (state === 'error')
      return <AlertIcon className={`${cls} text-destructive`} />
    return <ZapIcon className={`${cls} text-amber-500`} />
  }
  if (state === 'calling')
    return <WrenchIcon className={`${cls} animate-pulse text-blue-500`} />
  if (state === 'error')
    return <AlertIcon className={`${cls} text-destructive`} />
...
function PhaseLabel({
  ...
  const label = isSkill
    ? {
        calling: 'Activating skill...',
        completed: 'Skill activated',
        error: 'Failed',
      }[state]
...
export function ToolCallBlock({ toolCall, compact }: ToolCallBlockProps) {
  ...
  return (
    <div
      className={compact ? '' : 'rounded-md border border-border bg-muted/30'}
    >
      <button
        type="button"
      ...
      >
        ...
        {summary && (
          <span className="truncate text-muted-foreground ml-1">{summary}</span>
        )}
        ...
        <div className="border-t border-border px-3 py-2 space-y-2">
          ...
          {Object.keys(toolCall.arguments).length > 0 && (
            <div>
              ...
              <pre className="whitespace-pre-wrap text-[11px] font-mono text-muted-foreground bg-card rounded p-1.5 max-h-40 overflow-y-auto">
