  ...
  return (
    <div className="mb-3 rounded-lg border border-border bg-muted/50 overflow-hidden">
      ...
      >
        {/* Brain icon */}
        <svg
          className={`w-3.5 h-3.5 shrink-0 ${isStreaming ? 'text-amber-500' : 'text-muted-foreground'}`}
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
      ...
      {expanded && (
        <div className="px-3 pb-3">
          <pre className="text-xs text-muted-foreground whitespace-pre-wrap leading-relaxed font-mono max-h-80 overflow-y-auto break-words">
            {text}
            {isStreaming && (
              <span className="inline-block w-1.5 h-3.5 bg-muted-foreground/50 animate-pulse rounded-sm ml-0.5 align-text-bottom" />
            )}
          </pre>
        </div>
      )}
