...
interface MemoryCreateDialogProps {
  ...
  knownUserIds?: string[];
  /** If provided, dialog operates in edit mode using putMemoryDocument */
  documentId?: number;
...
export function MemoryCreateDialog({
  ...
  const { createDocument, updateDocument } = useMemoryDocuments({ agentId: effectiveAgentId || undefined });
  ...
  }, [open, parentAgentId, initialPath, initialContent, reset]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditMode) {
        await updateDocument(documentId!, data.content.trim());
      } else {
        await createDocument(data.path.trim(), data.content.trim());
      }
      onSuccess?.();
      onOpenChange(false);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !loading && onOpenChange(v)}>
      <DialogContent aria-describedby={undefined} className="max-w-3xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>{isEditMode ? t("createDialog.editTitle", { defaultValue: "编辑文档" }) : t("createDialog.title")}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={loading}>
            {loading ? t("createDialog.creating") : isEditMode ? t("createDialog.save", { defaultValue: "保存" }) : t("createDialog.create")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
