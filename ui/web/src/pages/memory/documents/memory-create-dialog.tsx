...
export function MemoryCreateDialog({ open, onOpenChange, agentId: parentAgentId, knownUserIds = [] }: MemoryCreateDialogProps) {
  ...
  useMemoryDocuments({ agentId: effectiveAgentId || undefined });
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      // Document creation is no longer supported
      setError("Creating new documents is not supported. Please edit existing documents.");
    } catch (err) {
