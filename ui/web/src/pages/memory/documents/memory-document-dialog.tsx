...
export function MemoryDocumentDialog({ open, onOpenChange, agentId, document }: MemoryDocumentDialogProps) {
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      const res = await http.get<{ document: MemoryDocument }>(
        `/v1/agents/${agentId}/memory/documents/${document.document_id}`,
      );
      const details = res?.document as MemoryDocumentDetail;
      if (!details) throw new Error("No document data");
      setDetail(details);
      setContent(details.content || document.content || "");
    } catch {
      toast.error(i18next.t("memory:toast.docLoadFailed"));
      // Use the passed document content as fallback
  ...
  const loadChunks = useCallback(async () => {
    ...
    try {
      ...
      const res = await http.get<{ chunks: MemoryChunk[] }>(
  ...
  }, [open, document, agentId, loadDetail]);

  useEffect(() => {
    if (tab === "chunks" && !chunksLoaded && !loadingChunks && document) {
      loadChunks();
    }
  }, [tab, chunksLoaded, loadingChunks, document, loadChunks]);

  const handleSave = async () => {
    if (!document || !agentId) return;
    setSaving(true);
    try {
      await http.put(`/v1/agents/${agentId}/memory/documents/${document.document_id}`, {
        content,
      });
    ...
    <Dialog open={open} onOpenChange={(v) => !saving && onOpenChange(v)}>
      <DialogContent aria-describedby={undefined} className="max-h-[85vh] w-[95vw] flex flex-col sm:max-w-4xl">
        ...
        <div className="flex border-b">
          ...
          >
            {chunks.length > 0 ? t("documentDialog.chunks", { count: chunks.length }) : t("documentDialog.noChunks")}
          </button>
        </div>
            ...
            <div className="grid gap-3">
              {detail && (
                <div className="flex gap-4 text-xs text-muted-foreground">
                  {detail.chunk_count !== undefined && (
                    <>
                      <span>{t("documentDialog.path")} {detail.chunk_count}</span>
                      <span>{detail.embedded_count ?? 0}/{detail.chunk_count}</span>
                    </>
                  )}
                  {detail.created_at && (
                  ...
                  {chunks.map((chunk) => (
                    <div key={chunk.id} className="rounded-md border p-3">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant={chunk.has_embedding ? "default" : "secondary"} className="text-2xs">
