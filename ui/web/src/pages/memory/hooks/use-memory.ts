...
export function useMemoryDocuments(filters: MemoryDocFilters) {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!filters.agentId) {
        const res = await http.get<{ documents: MemoryDocument[] }>("/v1/memory/documents");
        return res?.documents ?? [];
      }
      const params: Record<string, string> = {};
      if (filters.userId) params.user_id = filters.userId;
      const res = await http.get<{ documents: MemoryDocument[] }>(
  ...
  const getDocument = useCallback(
    async (documentID: number) => {
      const res = await http.get<{ document: MemoryDocument }>(
        `/v1/agents/${filters.agentId}/memory/documents/${documentID}`,
      );
      return res?.document as MemoryDocumentDetail | undefined;
    },
  ...
  const updateDocument = useCallback(
    async (documentID: number, content: string) => {
      try {
        await http.put(`/v1/agents/${filters.agentId}/memory/documents/${documentID}`, {
          content,
        });
        await invalidate();
  ...
  const deleteDocument = useCallback(
    async (documentID: number) => {
      ...
      try {
        await http.delete(`/v1/agents/${filters.agentId}/memory/documents/${documentID}`);
        await invalidate();
